# Phase 4: CLI + HTTP API

> **版本**：0.9
> **目标**：后端完整可用——CLI 命令接口 + HTTP/WebSocket API
> **预估周期**：1-2 周
> **前置依赖**：Phase 3 (Relay) 完成
...
### TC-4-CLI-001: ezagent init 注册身份
...
### TC-4-CLI-002: ezagent init 指定 CA 证书
...
### TC-4-CLI-003: ezagent init 重复注册拒绝
...
### TC-4-CLI-004: ezagent identity whoami
...
### TC-4-CLI-005: ezagent identity whoami 未初始化
...
### TC-4-CLI-010: ezagent room create
...
### TC-4-CLI-011: ezagent rooms 列表（table 格式）
...
### TC-4-CLI-012: ezagent rooms --json

```
GIVEN  同 TC-4-CLI-011
...
### TC-4-CLI-013: ezagent rooms --quiet
...
### TC-4-CLI-014: ezagent room show
...
### TC-4-CLI-015: ezagent room invite
...
### TC-4-CLI-016: ezagent room show 不存在的 Room
...
### TC-4-CLI-021: ezagent send 非成员被拒
...
### TC-4-CLI-022: ezagent messages 列表
...
### TC-4-CLI-023: ezagent messages --limit 分页
...
### TC-4-CLI-024: ezagent messages --before 游标分页
...
### TC-4-CLI-030: ezagent events 实时流
...
### TC-4-CLI-031: ezagent events --room 过滤
...
### TC-4-CLI-032: ezagent events --json
...
### TC-4-CLI-041: ezagent status Relay 不可达
...
### TC-4-CLI-042: ezagent start 启动 HTTP Server
...
### TC-4-CLI-043: ezagent start --no-ui
...
### TC-4-HTTP-001: GET /api/identity
...
### TC-4-HTTP-002: GET /api/identity/{entity_id}/pubkey
...
### TC-4-HTTP-003: GET /api/identity/{entity_id}/pubkey 不存在
...
### TC-4-HTTP-010: POST /api/rooms 创建
...
### TC-4-HTTP-011: GET /api/rooms 列表
...
### TC-4-HTTP-012: GET /api/rooms/{room_id} 详情
...
### TC-4-HTTP-013: PATCH /api/rooms/{room_id} 更新
...
### TC-4-HTTP-014: POST invite + GET members
...
### TC-4-HTTP-015: POST join + POST leave
...
### TC-4-HTTP-020: POST /api/rooms/{room_id}/messages 发送
...
### TC-4-HTTP-021: GET /api/rooms/{room_id}/messages 分页
...
### TC-4-HTTP-022: GET /api/rooms/{room_id}/messages/{ref_id} 单条
...
### TC-4-HTTP-031: GET Annotation 列表
...
### TC-4-HTTP-032: DELETE Annotation 权限
...
### TC-4-HTTP-042: EXT-03 Reaction 添加/移除
...
### TC-4-HTTP-043: EXT-06 Channel 列表
...
### TC-4-HTTP-044: EXT-06 Channel 聚合视图
...
### TC-4-HTTP-045: EXT-07 Moderation
...
### TC-4-HTTP-046: EXT-08 Read Receipts
...
### TC-4-HTTP-047: EXT-09 Presence + Typing
...
### TC-4-HTTP-048: EXT-10 Media 上传/下载
...
### TC-4-HTTP-050: EXT-11 Thread 视图
...
### TC-4-HTTP-051: EXT-12 Drafts（私有）
...
### TC-4-HTTP-052: EXT-13 Profile GET/PUT
...
### TC-4-HTTP-053: EXT-14 Watch CRUD
...
### TC-4-HTTP-054: EXT-05 Cross-Room Preview
...
### TC-4-HTTP-055: EXT-02 Collab ACL
...
### TC-4-HTTP-060: GET /api/renderers 全局
...
### TC-4-HTTP-061: GET /api/rooms/{room_id}/renderers
...
### TC-4-HTTP-062: GET /api/rooms/{room_id}/views
...
### TC-4-CLI-URI-001: ezagent open 解析 Room URI
...
### TC-4-CLI-URI-002: ezagent open 解析 Message URI
...
### TC-4-CLI-URI-003: ezagent open 无效 URI
...
### TC-4-CLI-URI-004: ezagent open 资源不存在
...
### TC-4-CLI-URI-005: ezagent open 规范化
...
| 区域 | 编号范围 | 数量 |
|------|---------|------|
| CLI — Identity | TC-4-CLI-001~005 | 5 |
| CLI — Room | TC-4-CLI-010~016 | 7 |
| CLI — Message | TC-4-CLI-020~024 | 5 |
| CLI — Events/System | TC-4-CLI-030~043 | 7 |
| CLI — Config/Exit | TC-4-CLI-050~054 | 5 |
| CLI — URI Navigation | TC-4-CLI-URI-001~005 | 5 |
| HTTP — Bus API | TC-4-HTTP-001~024 | 15 |
| HTTP — Annotation | TC-4-HTTP-030~032 | 3 |
| HTTP — Extension | TC-4-HTTP-040~055 | 16 |
| HTTP — Render API | TC-4-HTTP-060~062 | 3 |
| WebSocket | TC-4-WS-001~006 | 6 |
| HTTP — Error/Status | TC-4-HTTP-070~074 | 5 |
| **合计** | | **82** |
