...
ezagent 采用 **P2P-First** 网络拓扑。每个实例内嵌 Zenoh peer，通过 multicast scouting 自动发现同网段节点并建立直连。Public Relay 仅在跨网络场景下作为桥接和持久化服务。URI 中的 authority（relay_domain）是身份命名空间，不是网络地址——同一资源在不同网络路径下 URI 不变（详见 §1.5）。
...
Entity 的 URI 表示详见 §1.5。

### 1.5 URI Scheme

> **来源**：[EEP-0001](../eep/EEP-0001.md)（ezagent URI Scheme）

ezagent 定义统一的 URI 标准，将三层架构的所有可寻址资源映射为标准化的外部标识符。URI 是 Deep Link、资源分享、跨实例引用等上层能力的寻址基础。
...
| 部分 | 格式 | 说明 |
|------|------|------|
| **Scheme** | `ezagent` | 固定。客户端 scheme handler 注册 |
| **Authority** | `{relay_domain}` | 身份命名空间标识，**不是网络地址** |
| **Path** | 见 §1.5.2 | 分层路径，资源类型 + 标识符 |
| **Query** | `key=value&...` | 可选。过滤、分页、时间范围等 |
| **Fragment** | `#{anchor}` | 可选。客户端行为提示 |

**Authority 语义**：

- [MUST] Authority 就是 Entity ID 中 `@local:` 后面的 `relay_domain`。它是**身份命名空间**，不是网络地址。客户端解析 URI 时，通过 DNS 解析 relay_domain 获取 Relay 地址，再通过 Relay 发现资源所在的 peer。
...
| 资源 | Path | 标识符来源 |
|------|------|-----------| 
| Identity | `/@{entity_id}` | §1.4 Entity ID |
| Room | `/r/{room_id}` | §4.2, UUIDv7 |
| Message | `/r/{room_id}/m/{ref_id}` | §4.4, ULID |
| Timeline | `/r/{room_id}/t` | §4.3 |
...
跨域引用：  ezagent://relay-a.com/@bob:relay-b.com
```
...
Extension 在 Mid-layer 路径下注册子路径，命名空间为 Extension ID。详见 extensions-spec §1.2。

| 资源 | Path | Extension |
|------|------|-----------|
| Channel 视图 | `/r/{room_id}/c/{channel_name}` | EXT-06 Channels |
...
- [MUST] Extension 注册 URI 子路径时，MUST 在 manifest.toml 中声明 `uri_paths`（详见 extensions-spec §1.2）。

**Socialware 资源**：

Socialware 资源通过 namespace 子路径访问：
...
**映射规则**：`ezagent://{authority}/{path}` → `https://{authority}/e/{path}`

- [MUST] Web fallback path MUST 以 `/e/` 为前缀，与 Relay 的 Admin API (`/admin/`) 和 Health (`/health`) 路径区分。
- [SHOULD] Relay 收到 Web fallback 请求时，SHOULD 返回 HTML 页面，包含资源的只读预览和 `ezagent://` deep link。
